...
impl VariableParser {
    ...
    fn parse_variable(&self, variable: (String, f64)) -> Variable {
        ...
        let position = key[self.tileset_digits()..self.tileset_digits() + self.sequence_digits()]
            .parse::<usize>()
            .unwrap();
        let placed_tile = PlacedTile { tile, position };
...
impl SolutionBuilder {
    pub fn build(self, puzzle: &Puzzle, variables_map: HashMap<String, f64>, n: usize) -> Solution {
        ...
        let variable_parser = VariableParser { n };
        let variables = variable_parser.parse(variables_map);
        for (_position, tile) in variables
            .into_iter()
            .filter(|variable| variable.value)
            .map(|variable| (variable.key.position, variable.key.tile))
        {

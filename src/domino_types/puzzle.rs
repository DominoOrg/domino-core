...
impl FitPuzzleChecker {
    ...
    pub fn check(&self, puzzle: &Puzzle, position: usize, tile: Tile) -> bool {
        match position {
            0 => self.check_right(puzzle, position, tile),
            position if position == puzzle.len() - 1 => self.check_left(puzzle, position, tile),
            position => {
                self.check_left(puzzle, position, tile) && self.check_right(puzzle, position, tile)
            }
        }
    }
}

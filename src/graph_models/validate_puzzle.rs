...
pub fn validate(puzzle: &Puzzle) -> Result<(), String> {
    ...
    let free_tiles = tileset
        .into_iter()
        .filter(|tile| !puzzle.vec.contains(&Some(*tile)))
        .collect::<Vec<Tile>>();
    let solution = solution.unwrap();
    let solution_tiles = solution
        .into_iter()
    ...
    for tile in free_tiles {
            ...
            .map(|tile| tile.0)
        {
            ...
            if let Some(altered_solution) = altered_solution {
                ...
                let altered_solution_tiles = altered_solution
                    .into_iter()
                    .map(|tile| Tile::try_from(tile).unwrap())
                    .collect::<Vec<Tile>>();

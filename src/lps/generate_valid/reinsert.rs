...
pub(super) fn reinsert_tile_and_check(
  ...
  let (new_puzzle, new_removed_tiles) = reinsert_tile(puzzle, removed_tiles, tile, position).unwrap();
...
fn select_tile_and_position(puzzle: &Puzzle, solution: &Solution, removed_tiles: &Vec<Tile>, random: bool) -> (Tile, usize) {
  match (random, removed_tiles.len() == solution.len()) {
      (true, true) => (
          random_tile(solution.clone()),
          random_empty_position(puzzle)
      ),
      (true, false) => (
...
fn reinsert_tile(puzzle: Puzzle, removed_tiles: Vec<Tile>, tile: Tile, position: usize) -> Result<(Puzzle, Vec<Tile>), String> {
...
mod tests {
  ...
  fn test_reinsert_tile_and_check() {
    ...
    if let Ok(solution) = solution {
      let is_valid = validate_puzzle(&puzzle, &solution).is_ok();
    ...
    //     let puzzle = super::reinsert_tile_and_check(
    //       vec![None; solution.clone().len()],
    //       solution.clone(),
    //       solution.clone(),
    //       c,
    //       random);
    ...
    //     assert_eq!(classify_puzzle(&puzzle), c);
    //     assert_eq!(puzzle.len(), solution.len());
    //   });
    // });
  ...
  fn test_reinsert_tile_with_valid_tile_and_valid_position() {
    ...
    let (puzzle, removed_tiles_after_removal) = super::reinsert_tile(puzzle, removed_tiles.clone(), tile, position).unwrap();

    // Test reinsertion on valid position with valid tile
    assert_eq!(puzzle, vec![Some(tile), None, None, None, None, None, None, None]);
    let mut expected_after_removal =  removed_tiles.clone();
    let tile_position = expected_after_removal.iter()
    .position(|&x| x == tile).unwrap();
    expected_after_removal
      .splice(
        tile_position..tile_position+1,
        vec![]);

...
pub fn variables(puzzle: &Puzzle) -> Result<Variables, DominoError> {
    ...
    let mapped_variables: Vec<Variable> = generate_combinations(tileset, n)
        .into_iter()
        .collect();
...
fn format_on_n_digits(number: usize, digits: usize) -> String {
    ...
    let padding_length = digits.saturating_sub(number_string.len());
    let padding = "0".repeat(padding_length);
    format!("{}{}", padding, number_string)
}
